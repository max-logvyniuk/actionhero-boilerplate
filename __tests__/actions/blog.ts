...
describe('postAdd test', () => {
    ...
    test('can add new post', async () => {
        ...
        expect(JSON.stringify(omit(response.post, ["createdAt", "updatedAt"]))).toEqual(JSON.stringify({
            content: "Hello everyone!!!",
            title: "task",
            userName: "Poll"
        }));
    ...
    test('can`t add new post', async () => {
        ...
        expect(response.error).toBe("Error: userName does not exist (TypeError: Cannot read property 'hashedPassword' of null)");
    })
});